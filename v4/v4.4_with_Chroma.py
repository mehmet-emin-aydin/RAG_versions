...
def ask(question, chat_history):
    ...
    def reciprocal_rank_fusion(results: list[list], k=60):
        ...
        for docs in results:
            ...
            for rank, doc in enumerate(docs):
                print(f'First results:{doc.metadata}')
                if doc.metadata not in metadatas:
        ...
        # st.session_state["retrieved_chunks"] = [scored_doc[0].metadata for scored_doc in reranked_results[:5]]
        chunk_info = [
            f"Chunk ID: {chunk[0].metadata['chunk_id']}\tPage: {chunk[0].metadata['page']}\tSource: {chunk[0].metadata['source']}\n"
    ...
    return final_rag_chain.stream({"question":question})
...
def process_uploaded_files(username, uploaded_files):
    ...
    for file in uploaded_files:
        ...
        if file_extension == '.txt':
            ...
            [docs.extend(Document(page_content=chunks[i], metadata = {'source':file.name, 'page': i+1}) for i in range(len(chunks)))]
        elif file_extension == '.pdf':
            reader = PyPDF2.PdfReader(io.BytesIO(file.read()))
            [docs.extend(Document(page_content=page.extract_text(), metadata = {'source':file.name, 'page': i+1}) for i, page in enumerate(reader.pages))]
    ...
    # [print(split_doc) for split_doc in split_docs]
    ...
    if "username" not in st.session_state:
        ...
        if "vectorstore_initialized" not in st.session_state or not st.session_state["vectorstore_initialized"]:
            ...
            if st.session_state["chat_answers_history"]:
                questions = st.session_state["user_prompt_history"]
                answers = st.session_state["chat_answers_history"]
            ...
            if prompt:
                ...
                with st.spinner("Generating......"):
                    ...
                    message2 = st.chat_message("assistant")
                    full_message = message2.write_stream(output)
                    st.session_state["chat_answers_history"].append(full_message)
                    st.session_state["user_prompt_history"].append(prompt)  
                    
                    
                    
                    st.session_state["chat_history"].append((prompt, full_message))